...
public class EntityTrent extends EntityMob {
    ...
    public EntityTrent(World world) {
        ...
        this.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.maxHealth).setBaseValue((double) (2000.0D));
        super.setHealth(2000.0F);
    ...
    public String getCommandSenderName() {
        return StatCollector.translateToLocal("entity.AdvanceWitchery.trent.name");
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float dmg) {
        if (source.getEntity() != null && source.getEntity() instanceof EntityPlayer) {
            int range = 10;
            List<EntityEnt> list = worldObj.getEntitiesWithinAABB(EntityEnt.class, AxisAlignedBB.getBoundingBox((posX - range), (posY - range), (posZ - range), (posX + range), (posY + range), (posZ + range)));
            if (list.size() > 0)
                return false;

            if (!worldObj.isRemote && worldObj.rand.nextDouble() <= 0.3D) {
                int count = 1 + worldObj.rand.nextInt(3);
                for (int i = 0; i < count; ++i) {
                    EntityEnt ee = new EntityEnt(worldObj);
                    ee.setPositionAndUpdate(posX, posY, posZ);
    ...
    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_) {
        this.entityDropItem(ModItems.itemHeart.create(2), 0);
        this.entityDropItem(ModItems.dropBlood.create(2), 0);
    ...
    public void onDeath(DamageSource source) {
        ...
        if (source.getSourceOfDamage() instanceof EntityPlayer && !worldObj.isRemote && !StatManager.hasStat(StatManager.elfBlood, (EntityPlayer) source.getSourceOfDamage()))
            ((EntityPlayer)source.getSourceOfDamage()).triggerAchievement(StatManager.killEnt);
    }
