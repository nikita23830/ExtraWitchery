...
public class EntityGuardian extends EntityMob implements IBossDisplayData {
    ...
    public EntityGuardian(World world) {
        ...
        this.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.maxHealth).setBaseValue((double) (6000.0F));
        super.setHealth(6000.0F);
    ...
    public String getCommandSenderName() {
        return StatCollector.translateToLocal("entity.AdvanceWitchery.guardian.name");
    }
    ...
    public boolean attackEntityAsMob(Entity par1Entity) {
        ...
        boolean flag = par1Entity.attackEntityFrom(DamageSource.magic, 10F);
    ...
    public boolean attackEntityFrom(DamageSource source, float dmg) {
        if (source.getSourceOfDamage() instanceof EntityPlayer) {
            if (worldObj.rand.nextDouble() <= .2D && !worldObj.isRemote) {
                ...
                do {
                    --count;
                    EntityVillageGuard guard = new EntityVillageGuard(worldObj);
                    guard.setTarget(source.getSourceOfDamage());
                    guard.setAttackTarget((EntityLivingBase) source.getSourceOfDamage());
                    guard.setRevengeTarget((EntityLivingBase) source.getSourceOfDamage());
                    guard.setPositionAndUpdate(posX, posY, posZ);
                    worldObj.spawnEntityInWorld(guard);
                } while (count >= 0);
            }
            return super.attackEntityFrom(source, Math.min(dmg, 100F));
        }
        return false;
    }
    ...
    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_) {
        this.entityDropItem(ModItems.fragmentNetherStar.create(), 0F);
        for (int i = 0; i < 4; ++i) {
            this.entityDropItem(ModItems.breakArmor.create(i), 0F);
        }
    }

    public void onDeath(DamageSource source) {
        super.onDeath(source);
        if (source.getSourceOfDamage() instanceof EntityPlayer && !worldObj.isRemote) {
            EntityPlayer ep = (EntityPlayer) source.getSourceOfDamage();
            if (StatManager.hasStat(StatManager.readFivePart, ep) && !StatManager.hasStat(StatManager.killGuardian, ep))
                ep.triggerAchievement(StatManager.killGuardian);
        }
    }
