...
public class EntityBeholder extends EntityMob {
    ...
    public String getCommandSenderName() {
        return StatCollector.translateToLocal("entity.AdvanceWitchery.beholder.name");
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float dmg) {
        if (source.getSourceOfDamage() instanceof EntityPlayer) {
            if (worldObj.rand.nextDouble() <= .2D && !worldObj.isRemote)
                ((EntityPlayer)source.getSourceOfDamage()).addPotionEffect(new PotionEffect(Potion.blindness.getId(), 201));
            return super.attackEntityFrom(source, Math.min(dmg, 140F));
        }
        return false;
    }
    ...
    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_) {
        this.entityDropItem(ModItems.demonEye.create(), 0F);
    }
