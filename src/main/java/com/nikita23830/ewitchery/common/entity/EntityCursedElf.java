...
public class EntityCursedElf extends EntityMob implements IBossDisplayData {

    private int attackTimer = 0;

    public EntityCursedElf(World world) {
        super(world);
        ...
        this.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.maxHealth).setBaseValue((double) (160.0F));
        super.setHealth(160.0F);
    ...
    public String getCommandSenderName() {
        return StatCollector.translateToLocal("entity.AdvanceWitchery.cursedElf.name");
    }
    ...
    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_) {
        this.entityDropItem(ModItems.itemHeart.create(), 0F);
        //this.dropItem(ModItems.itemHeart, 0);
    }

    public void onDeath(DamageSource source) {
        super.onDeath(source);
        if (worldObj.rand.nextDouble() <= 0.6D && !worldObj.isRemote) {
            EntityAsgard ea = new EntityAsgard(worldObj);
            ea.setPositionAndRotation(posX, posY, posZ, rotationYaw, rotationPitch);
